...
// font5x7 stores each character as 7 rows of 5 bits (one byte per row, bits 4-0 = cols left to right)
...
func DrawText(c *rgbmatrix.Canvas, x, y int, text string, col color.RGBA) {
	...
	for _, ch := range text {
		...
		// Each byte is one row, bits 4-0 are columns left to right
		for row := 0; row < 7; row++ {
			for col_idx := 0; col_idx < 5; col_idx++ {
				if glyph[row]&(1<<uint(4-col_idx)) != 0 {
					px := cx + col_idx
					py := y + row
